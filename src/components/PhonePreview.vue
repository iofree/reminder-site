<template>
  <div class="iphonePreview">
    ...
    <div v-if="showVideo" class="videoContainer">
      <video class="screenvideo" autoplay controls>
        <source v-for="videoSrc in videoSources" :key="videoSrc.src" :src="videoSrc.src" :type="videoSrc.type">
      </video>
    </div>

    <img v-if="!showVideo && screenshotSrc" class="iphoneScreen" :src="screenshotSrc" alt="App Screenshot">
  </div>
</template>
...
const videoSources = ref<Array<{src: string, type: string}>>([])

onMounted(() => {
  // 检查是否有截图
  const screenshots = ['/assets/screenshot/Default-2688h.png']
  if (screenshots.length > 0) {
    screenshotSrc.value = screenshots[0]
  }
